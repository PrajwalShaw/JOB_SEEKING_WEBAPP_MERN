...
export const jobseekerGetAllApplications = catchAsyncError(async(req,res,next)=>{
    const {role} = req.user;
    if(role === "Employer"){
        return next(new ErrorHandler("Employer is not allowed to access these resources"));
    }
...
export const jobSeekerDeleteApplication = catchAsyncError(async(req,res,next)=>{
    ...
    if(!application){
        return next(new ErrorHandler("Oops job application does not exist",404));
    }
...
export const postApplication = catchAsyncError(async(req,res,next)=>{
    ...
    if(!allowedFormats.includes(resume.mimetype)){
        return next(new ErrorHandler("Please upload file in PNG,JPG or WEBP format",400));//img.png...this png is the mimetype
    }
